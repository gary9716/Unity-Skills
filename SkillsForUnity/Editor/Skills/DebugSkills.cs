...
namespace UnitySkills
{
    ...
    public static class DebugSkills
    {
        ...
        internal const int ErrorModeMask =
            ModeError
            | ModeAssert
            | ModeFatal
            | ModeAssetImportError
            | ModeScriptingError
            | ModeScriptCompileError
            | ModeScriptingException;
        internal const int WarningModeMask =
        ...
        internal static bool EnsureReflection()
        {
            ...
            if (_logEntriesType == null || _logEntryType == null)
            {
                SkillsLogger.LogError(
                    "DebugSkills: UnityEditor.LogEntries or LogEntry type not found."
                );
                return false;
            }
            ...
            {
                SkillsLogger.LogError(
                    "DebugSkills: Failed to reflect required members of LogEntries/LogEntry."
                );
        ...
        /// <summary>
        /// Split Unity's combined "message\nstackTrace" string into parts.
        /// Unity embeds the stack trace in the message field separated by newlines.
        /// Returning them as separate fields avoids literal newlines in JSON strings,
        /// which break parsers when Newtonsoft serializes them with StringEscapeHandling.Default.
        /// </summary>
        private static (string message, string stackTrace) SplitMessageAndStack(string raw)
        {
            ...
            if (nl < 0)
                return (raw, "");
            var firstLine = raw.Substring(0, nl).Trim();
            var rest = raw.Substring(nl + 1).Trim();
            // Collapse remaining newlines so stack trace is a single safe string
            var stack = rest.Replace("\n", " | ");
            return (firstLine, stack);
        }
        ...
        internal static List<LogEntryInfo> ReadLogEntries(int targetMask, string filter, int limit)
        {
            ...
            try
            {
                ...
                for (int i = count - 1; i >= 0 && found < limit; i--)
                {
                    ...
                    string raw = (string)_messageField.GetValue(entry) ?? "";
                    if (!string.IsNullOrEmpty(filter) && !raw.Contains(filter))
                        continue;
                    ...
                    string logType =
                        (mode & ErrorModeMask) != 0 ? "Error"
                        : (mode & WarningModeMask) != 0 ? "Warning"
                        : "Log";

                    var (msg, stack) = SplitMessageAndStack(
        ...
        public static object DebugGetLogs(
            string type = "Error",
            string filter = null,
            int limit = 50
        )
        {
            ...
            if (type.Contains("Error"))
                targetMask |= ErrorModeMask;
            if (type.Contains("Warning"))
                targetMask |= WarningModeMask;
            if (type.Contains("Log"))
                targetMask |= LogModeMask;
            if (targetMask == 0)
                targetMask = ErrorModeMask;
        ...
        public static object DebugGetMemoryInfo()
        {
            ...
            {
                success = true,
                totalAllocatedMB = UnityEngine.Profiling.Profiler.GetTotalAllocatedMemoryLong()
                    / (1024.0 * 1024.0),
                totalReservedMB = UnityEngine.Profiling.Profiler.GetTotalReservedMemoryLong()
                    / (1024.0 * 1024.0),
                totalUnusedReservedMB = UnityEngine.Profiling.Profiler.GetTotalUnusedReservedMemoryLong()
                    / (1024.0 * 1024.0),
                monoUsedSizeMB = UnityEngine.Profiling.Profiler.GetMonoUsedSizeLong()
                    / (1024.0 * 1024.0),
                monoHeapSizeMB = UnityEngine.Profiling.Profiler.GetMonoHeapSizeLong()
                    / (1024.0 * 1024.0),
            };
        }
    }
